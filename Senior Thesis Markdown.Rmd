...
for (i in 1:5){ # the lags - 1 to 5 year lags
  for (t in 1978:2015){ # the data time range
    ...
    CRSP_Compustat_for[[vars[i]]] = ifelse((fyear == t)*(rds_nm_ind >= 6)*(perc_pos>=0.5), (sum_yx/sum_xx), NA)
  }
}
```
...
```{r}
C <- CRSP_Compustat_for
vars <- c("b1", "b2", "b3", "b4", "b5")
for (i in 1:5){ # the lags - 1 to 5 year lags
  for (t in 1978:2015){ # the data time range
      reg_period = (C['fyear']<=t)*(C['fyear']>=(t-7))
      x_use = ifelse(reg_period, lag(C['RDI'], i), NA)
      y_use = ifelse(reg_period, C['sales_growth'], NA)
      x_use = ifelse(is.na(y_use), NA, x_use)
      y_use = ifelse(is.na(y_use), NA, y_use)
      
      RDI_nm_chk = (x_use<NA)*reg_period
      RDI_pos_chk = (x_use<NA)*(x_use>0)*reg_period
...
vars <- c("b1", "b2", "b3", "b4", "b5")
for (i in 1:5){
  CRSP_Compustat_for[[vars[i]]] = 1
}

# Don't use mutate. Use the [[]] instead to update the value because you already created the columns.
for (i in 1:5){
  CRSP_Compustat_for[[vars[i]]] <- 1
}
```
...
```{r}
# To update the values of b1, ... b5

for (i in 1:5) {
  varname <- paste0("b",i) # varname is b1, ..., b5 for each i
  for (t in 1978:2015) {
    if ( (fyear==t) & (rds_nm_ind>=6) & (perc_pos>=.5) ) {
      CRSP_Compustat_for[[varname]] <- sum_yx/sum_xx
    }
  }
}
```
